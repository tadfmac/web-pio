// web-pio
// pio.cpp (ESP32 / BLE-MIDI version)
...
#endif
uint8_t i2cLock = 0;
uint8_t i2cAddrDiv = 0;
#define I2CADDRDIV_LIMIT 16
...
void initI2CSlaveStatus(){
  Serial.println("initI2CSlaveStatus");
  for(int cnt=0;cnt<128;cnt++){
    i2cSlaveStatus0[cnt] = 0;
  }
#ifdef WIRE1_ENABLE
  Serial.println("WIRE1_ENABLE");
  for(int cnt=0;cnt<128;cnt++){
...
#ifdef DEB
  Serial.print("RawInputData: ");
  for (size_t i = 0; i < resLen; i++) {
    Serial.print(rawInputData[i], HEX); Serial.print(" ");
  }
  Serial.println();
#endif
...
void processMessage(uint8_t *pMes, size_t _mesSize){
  ...
  uint8_t msb;
  uint16_t wordData;
  uint8_t doRespond = (pMes[0] == 1) ? 1 : 0;

  if(pMes[0] == 1 || pMes[0] == 0){
    ...
    case GPIO_EXPORT:
#ifdef DEB
      Serial.println("GPIO_EXPORT received");
#endif
    ...
    case GPIO_WRITE:
#ifdef DEB
      Serial.println("GPIO_WRITE received");
#endif
    ...
    case GPIO_READ:
#ifdef DEB
      Serial.println("GPIO_READ received");
#endif
    ...
    case GPIO_UNEXPORT:
#ifdef DEB
      Serial.println("GPIO_UNEXPORT received");
#endif
...
#ifdef DEB
      Serial.println("GPIO_UNEXPORTALL received");
#endif
      res = gpioUnexportAll();
      rawOutputData[MES_B_IDX+1] = res;
      _size = MES_H_SIZE+1;
    ...
    case GPIO_SETPWM:
#ifdef DEB
      Serial.println("GPIO_SETPWM received");
#endif
...
#ifdef DEB
      Serial.println("GPIO_ANALOGREAD received");
#endif
    ...
    case I2C_INIT:
#ifdef DEB
      Serial.println("I2C_INIT received");
#endif
    ...
    case I2C_WRITE8:
#ifdef DEB
      Serial.println("I2C_WRITE8 received");
#endif
    ...
    case I2C_WRITE16:
#ifdef DEB
      Serial.println("I2C_WRITE16 received");
#endif
    ...
    case I2C_WRITEBYTE:
#ifdef DEB
      Serial.println("I2C_WRITEBYTE received");
#endif
    ...
    case I2C_WRITEBYTES:
#ifdef DEB
      Serial.println("I2C_WRITEBYTES received");
#endif
    ...
    case I2C_READ8:
#ifdef DEB
      Serial.println("I2C_READ8 received");
#endif
      uint8_t readData;
    ...
    case I2C_READ16:
#ifdef DEB
      Serial.println("I2C_READ16 received");
#endif
    ...
    case I2C_READBYTE:
#ifdef DEB
      Serial.println("I2C_READBYTE received");
#endif
    ...
    case I2C_READBYTES:
#ifdef DEB
      Serial.println("I2C_READBYTES received");
#endif
    ...
    case I2C_PORTSCAN:
#ifdef DEB
      Serial.println("I2C_PORTSCAN received");
#endif
      if(pMes[MES_B_IDX+1] == 1){
#ifdef WIRE1_ENABLE
        Serial.println("I2C_PORTSCAN WIRE1_ENABLE");
        rawOutputData[MES_B_IDX+1] = 1;
#else
        Serial.println("I2C_PORTSCAN PORT=1 ERROR");
        rawOutputData[MES_B_IDX+1] = 0;
#endif
      ...
      if(rawOutputData[MES_B_IDX+1] == 1){
        Serial.print("I2C_PORTSCAN TRY RES=");
...
#ifdef DEB
  Serial.print("rawOutData: ");
  for (uint16_t i = 0; i < _size; i++) {
    Serial.print(pMess[i], HEX); Serial.print(" ");
  }
  Serial.println();
#endif
...
#ifdef DEB
  Serial.print("sysExOutData: ");
  for (uint16_t i = 0; i < sendLen; i++) {
    Serial.print(sysExOutData[i], HEX); Serial.print(" ");
  }
  Serial.println();
#endif
...
void sendGpioOnChange(uint8_t pinNumber,uint8_t onOff,uint8_t isPullup){
  ...
  rawOutputData[2] = (uint8_t)(sessionId & 0x00FF);
  rawOutputData[3] = (uint8_t)(sessionId >> 8);
  rawOutputData[4] = 0;
  rawOutputData[5] = 0;
  rawOutputData[6] = 0;
  rawOutputData[7] = 0;
...
uint8_t gpioWrite(uint8_t portNum, uint8_t value){
  ...
  if(idx == -1) return 0;
  if(pinStatus[idx] == 1){
    digitalWrite(portNum, value == 1 ? HIGH : LOW);
    return 1;
  }
  return 0;
}
...
uint8_t gpioUnexportAll(){
  for(int cnt=0;cnt<pinNums;cnt++){
    if(pinStatus[cnt] != 0){
      pinMode(pinNumbers[cnt], INPUT_PULLDOWN);
      pinStatus[cnt] = 0;
    }
  }
  return 1;
...
uint8_t gpioAnalogRead(uint8_t portNum, uint8_t *pLsb,uint8_t *pMsb){
  ...
  if(idx == -1) return 0;
  if(pinStatus[idx] == 5){
    int data = analogRead(portNum);
    *pLsb = (uint8_t)(data & 0x00FF);
    *pMsb = (uint8_t)((data >> 8) & 0x03);
    return 1;
  }
  return 0;
}
...
int getIndexFromPortNum(uint8_t portNum){
  for(int cnt=0;cnt<pinNums;cnt++){
    if(pinNumbers[cnt] == portNum) return cnt;
  }
  return -1;
}
...
uint8_t i2cDeviceInit(uint8_t portNum, uint8_t address){
...
#ifdef WIRE1_ENABLE
  if(portNum > 1) return 0;
  pWire = (portNum == 1) ? &Wire1 : &Wire;
  pStatus = (portNum == 1) ? i2cSlaveStatus1 : i2cSlaveStatus0;
#else
  ...
  uint8_t result = pWire->endTransmission();
  i2cLock = 0;
...
uint8_t i2cDeviceRead8(uint8_t portNum, uint8_t address, uint8_t reg, uint8_t *pOut){
  ...
  for(int cnt=0;cnt<readSize;cnt++) dmy = pWire->read();
  i2cLock = 0;
  return 0;
}
...
uint8_t i2cDeviceReadBytes(uint8_t portNum, uint8_t address, uint8_t size, uint8_t *pOut){
  ...
  if(size == readSize){
    for(int cnt=0;cnt<readSize;cnt++) *(pOut+cnt) = pWire->read();
    i2cLock = 0;
    return 1;
  }
  for(int cnt=0;cnt<readSize;cnt++) dmy = pWire->read();
...
uint8_t i2cDeviceWrite16(uint8_t portNum, uint8_t address, uint8_t reg, uint16_t data){
  ...
  pWire->beginTransmission(address);
  pWire->write(reg);
  pWire->write((uint8_t)(data & 0x00ff));
  pWire->write((uint8_t)(data >> 8));
  uint8_t result = pWire->endTransmission();
  i2cLock = 0;
  return (result == 0) ? 1 : 0;
}
...
uint8_t i2cDeviceWriteBytes(uint8_t portNum, uint8_t address, uint8_t length, uint8_t *pdata){
  ...
  i2cLock = 1;
  pWire->beginTransmission(address);
  for(int cnt=0;cnt<length;cnt++) pWire->write(*(pdata+cnt));
  uint8_t result = pWire->endTransmission();
  i2cLock = 0;
  return (result == 0) ? 1 : 0;
}
