...
export class emuViewer {
  ...
  getPins() {
    return {
      left:  this._config.leftPins.map(p => ({gpio: p.gpio, label: p.label})),
      right: this._config.rightPins.map(p => ({gpio: p.gpio, label: p.label})),
    };
  }
  ...
  _fitToContainer() {
    ...
    // Measure col width at a fixed reference size, assume linear scaling colW(f) ≈ k*f.
    // Then solve analytically for the largest f satisfying both the gap constraint
    // (badge rows don't overlap) and the width constraint (cols + SVG fit in domW).
    const fRef = 11;
    this._root.style.fontSize = fRef + 'px';
    const colRef = this._measureMaxColWidth();
    const k = colRef / fRef;

    // Substituting svgH = (domW - 2kf)/svgAr into gap constraint f = gapFactor*svgH - 2:
  ...
  _measureMaxColWidth() {
    const allPins = [...this._config.leftPins, ...this._config.rightPins];
    const maxLabel = allPins.reduce((a, b) => b.label.length > a.label.length ? b : a).label;
